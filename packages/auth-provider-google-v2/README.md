...
```bash
opam install auth-provider-google-v2
```
...
module Google_auth = Auth_provider_core.Make_oauth2_flow(Your_http_client)
...
```ocaml
let google_config = 
  let base_config = Auth_provider_google_v2.make_config
    ~client_id:"..."
    ~client_secret:"..."
    ~redirect_uri:"..."
    ()
...
module Lwt_http_client = struct
  let post ~url ~headers ~body ~on_success ~on_error =
    Lwt.async (fun () ->
      ...
      let response = Auth_provider_core.{
        status;
...
module Google_auth = Auth_provider_core.Make_oauth2_flow(Lwt_http_client)
