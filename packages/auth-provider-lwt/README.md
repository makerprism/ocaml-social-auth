...
```bash
opam install auth-provider-lwt auth-provider-google-v2
```
...
let authenticate_with_google () =
  ...
  let config = Auth_provider_google_v2.make_config
  ...
  let* result = complete_oauth_flow
    ...
    ~parse_user_info:Auth_provider_google_v2.parse_user_info
  in
...
let google_config = Auth_provider_google_v2.make_config
...
let google_login_handler _req =
  let (oauth_state, auth_url) = Auth_provider_lwt.start_authorization_flow google_config in
...
```ocaml
let* result = Auth_provider_lwt.exchange_code_for_tokens config ~code ~code_verifier in
match result with
| Ok token ->
    (* Success *)
...
let authenticate_user provider_type =
  let config = match provider_type with
    | `Google ->
        Auth_provider_google_v2.make_config
          ~client_id:(Sys.getenv "GOOGLE_CLIENT_ID")
          ~client_secret:(Sys.getenv "GOOGLE_CLIENT_SECRET")
          ~redirect_uri:"http://localhost:8080/callback"
          ()
    | `GitHub ->
        Auth_provider_github_v2.make_config  (* Future package *)
  ...
  let parse_user_info = match provider_type with
    | `Google -> Auth_provider_google_v2.parse_user_info
    | `GitHub -> Auth_provider_github_v2.parse_user_info
  in
  
  let (oauth_state, auth_url) = Auth_provider_lwt.start_authorization_flow config in
  (* ... rest of flow *)
```

## HTTP Client

The package uses `cohttp-lwt-unix` for HTTP requests. If you need a different HTTP client, you can implement the `HTTP_CLIENT` interface from `auth-provider-core` directly.
