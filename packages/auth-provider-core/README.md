...
```bash
opam install auth-provider-core
```
...
```ocaml
module Lwt_http_client : Auth_provider_core.HTTP_CLIENT = struct
  open Lwt.Syntax
...
let parse_google_user_info json_body =
  try
    ...
    let user_info = Auth_provider_core.{
      provider = Google;
...
```ocaml
module Google_oauth = Auth_provider_core.Make_oauth2_flow(Lwt_http_client)
...
│   └── auth_provider_core.ml  # Main module
├── dune-project
└── README.md
```
...
- `auth-provider-lwt` - Lwt runtime adapter (coming soon)
- `auth-provider-google-v2` - Google OAuth provider (coming soon)
- `auth-provider-github-v2` - GitHub OAuth provider (coming soon)
