# social-auth-core Design Document

## Overview

`social-auth-core` is a runtime-agnostic, HTTP-client-agnostic OAuth2 authentication library for OCaml. It provides the foundation for building secure OAuth2 login flows with any identity provider.
...
**Packages**:
```
social-auth-core/          # Core abstractions (this package)
social-auth-lwt/           # Lwt runtime adapter (future)
social-auth-google-v2/     # Google OAuth (future)
social-auth-github-v2/     # GitHub OAuth (future)
```
...
│   └── get_user_info
└── social_auth_core.ml  # Main module (re-exports)
```
...
1. **Provider Packages**: Create `social-auth-google-v2`
2. **Lwt Adapter**: Create `social-auth-lwt` with Cohttp implementation
